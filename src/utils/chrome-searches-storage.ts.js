...
export const setVintedDomainInStorage = async (value) => {
  await chrome.storage.sync.set({ vintedDomain: value });
};
export const getSearchesPreference = async () => {
  ...
  if (savedSearchPreference) {
    ...
    await chrome.storage.sync.set({ replaceDefaultSavedSearch: true });
  }
  return result;
};
...
export function parseQuery(url) {
  ...
  let query = match?.length > 0 ? match[match.length - 1] : "";
  query = query.replace(/time(.+?(&))/, "");
  query = query.replace(/page(.+[0-9])/, "");
...
export const checkSavedSearch = async (url) => {
  const list = await getSearchesList();
  const queryList = list.map(({ query }) => query);
  return queryList.includes(parseQuery(url));
};
